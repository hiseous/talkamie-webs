...
type ScheduledCardVar2Props = ComponentPrimitiveProps & {
    item: scheduleProps;
};

const ScheduledCardVar2 = (props: ScheduledCardVar2Props) => {
    ...
    return (
        ...
        >
            ...
            <Button
                theme={props.item.status === 'on-going' ? 'red' : 'pink'}
                className="mt-6 block text-center rounded-md"
                href={
                    props.item.status === 'on-going' && props.item.attendee?.id ? routes.user(props.item.attendee.id, ['chat']) :
                    props.item.id ? routes.schedule(props.item.id) :
                    undefined
                }
            >
                {
                    props.item.status === 'on-going' ? `Join Call` :
                    props.item.status === 'upcoming' ? `Reschedule` :
                    `View Details`
                }
            </Button>
        </ModalWrapperVar1>
...
export default ScheduledCardVar2;